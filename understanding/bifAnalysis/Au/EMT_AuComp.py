...
DSargsF.pars = {	'gz':gz, 'u0':u0,'nu':nu,'kz':kz,
		...
		'lamdaSu3':lamdaSu3, 'ku3':ku3, 'gs':gs, 'ks':ks, 'lamdaAu':0.6, 'nAu':1., 'A0Au':100. ,'A':300 ,'h':300}
	
DSargsF.varspecs = { 'u':'gu*H(Z(mz,u,u0,nu,kz,gz),Z0u,nzu,lamdazu)*H(S,S0u,nsu,lamdaSu)*H(h,1,1,1)*H(A,A0Au,nAu,lamdaAu)-mz*Yu(u,u0,nu)-ku*u',
		   'mz':'gmz*H(Z(mz,u,u0,nu,kz,gz),Z0m,nzm,lamdaZm)*H(S,S0m,nsm,lamdaSm)-mz*Ym(u,u0,nu)-kmz*mz',
		   'ms':'gms*H(h,1,1,1)*H(S,S0ms,nsms,lamdaSms)-ms*Ym3(u3,u30,nu3)-kms*ms',
...
##############
def plotCC(x,y):
	cl= ['b','r','k','g','m','y','orange']
	el= [0.,0.01,0.05,0.1,0.15,1.]
	#el= [1.,3.,5.,7.,9.,10.]
	### is this activation or inhibition
	ll,labs=[],[]
	minv,maxv=1000000,0
	for i in range(len(el)):
		DSargsF.pars['lamdaAu']=el[i]
...
if myrank==0:
	fig = plt.figure()
	[ll,labs,maxv,minv]=plotCC('A','mz')
	plt.title('')
	plt.xlim(0,3000)
	plt.ylim(minv,maxv)
	plt.xlabel("AMPK (molecules)",fontsize=20)
	plt.ylabel("Zeb mRNA (molecules)",fontsize=20)
	plt.legend(ll,labs,fontsize=2,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AU_hmz.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==1:
	fig = plt.figure()
	[ll,labs,maxv,minv]=plotCC('A','S')
	plt.title('')
	plt.xlim(0,3000)
	#plt.ylim(minv,maxv)
	plt.xlabel("AMPK (molecules)",fontsize=20)
	...
	plt.savefig("EM_AU_hs.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==2:
	fig = plt.figure()
	[ll,labs,maxv,minv]=plotCC('A','u')
	plt.title('')
	plt.xlim(0,3000)
	#plt.ylim(minv,maxv)
	plt.xlabel("AMPK (molecules)",fontsize=20)
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AU_hu.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==3:
	fig = plt.figure()
	[ll,labs,maxv,minv]=plotCC('A','ms')
	plt.title('')
	plt.xlim(0,3000)
	#plt.ylim(minv,maxv)
	plt.xlabel("AMPK (molecules)",fontsize=20)
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AU_hms.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==4:
	fig = plt.figure()
	[ll,labs,maxv,minv]=plotCC('A','u3')
	plt.title('')
	plt.xlim(0,3000)
	#plt.ylim(minv,maxv)
	plt.xlabel("AMPK (molecules)",fontsize=20)
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AU_hu3.png",bbox_inches='tight')
	#plt.show()
	plt.close()
