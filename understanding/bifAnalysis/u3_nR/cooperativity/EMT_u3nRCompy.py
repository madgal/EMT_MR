...
DSargsF.pars = { 'ga':ga,'ka':ka,'A0aa':A0aa,'A0ah':A0ah,'A0aR':A0aR,'A0rn':A0rn,'A0rm':A0rm,'lambdaaa':lambdaaa,'lambdaah':lambdaah,'lambdaar':lambdaar,'y':y,
		...
		'y':8,'kh':0.25,'u0uh':10000.,'nuh':1,'lamdauh':0.8,'u0u3R':10000,'nu3nR':4,'lamdau3nR':0.4,'u3':1000,'u':1000
		}
...
DSargsF.varspecs = { 'A':'ga*H(R(Rmt,Rnox),R0ra,nra,lambdara)*H(h,h0ha,nha,lambdaha)*H(A,A0aa,naa,lambdaaa)-ka*A',
		   ...
		   'Rnox':'grn*CompRn(gn,h,h0hrn,nhrn,g1,A,A0rn,g2,narn)-krn*Rnox*H(u3,u0u3R,nu3nR,lamdau3nR)',
...
DSargsF.xdomain = {'A':[0,5000],'h':[0,5000],'Rmt':[0,5000],'Rnox':[0,5000]}
...
##############
##############
def plotCC(x,y,fp='u3',check=False):
	cl= ['b','m','g','k','r','y','orange']
	el= [1,2,3,4,5,6,7]
	ll,labs=[],[]
	minv,maxv=1000000,0
	for i in range(len(el)):
		DSargsF.pars['nu3nR']=el[i]
...
if myrank==0:
	fig = plt.figure()
	[ll,labs,minv,maxv]=plotCC('u3','h')
	plt.title('')
	#plt.xlim(0,1500)
	plt.ylim(ymin=0)#minv,maxv)
	plt.xlabel("u34 ",fontsize=20)
	plt.ylabel("HIF1 ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3nR_3h.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==1:
	fig = plt.figure()
	[ll,labs,minv,maxv]=plotCC('u3','A')
	plt.title('')
	#plt.xlim(0,1500)
	plt.ylim(ymin=0)#minv,maxv)
	plt.xlabel("u34 ",fontsize=20)
	plt.ylabel("AMPK ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3nR_3A.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==2:
	...
	plt.ylabel("HIF1 ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3nR_AH.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==3:
	fig = plt.figure()
	[ll,labs,minv,maxv]=plotCC('u3','Rnox')
	plt.title('')
	#plt.xlim(0,1500)
	plt.ylim(ymin=0)#minv,maxv)
	plt.xlabel("u34 ",fontsize=20)
	plt.ylabel("noxRos ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3nR_3rn.png",bbox_inches='tight')
	#plt.show()
	plt.close()

if myrank==4:
	fig = plt.figure()
	[ll,labs,minv,maxv]=plotCC('u3','Rmt')
	plt.title('')
	#plt.xlim(0,1500)
	plt.ylim(ymin=0)#minv,maxv)
	plt.xlabel("u34 ",fontsize=20)
	plt.ylabel("mtROS ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3nR_3rm.png",bbox_inches='tight')
	#plt.show()
	plt.close()
