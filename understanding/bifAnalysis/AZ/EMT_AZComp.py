...
DSargsF.pars = {	'gz':gz, 'u0':u0,'nu':nu,'kz':kz,
		...
		'lamdaSu3':lamdaSu3, 'ku3':ku3, 'gs':gs, 'ks':ks, 'lamdaAm':7, 'nAm':2., 'A0m':100. ,'A':300 ,'h':300}
	
DSargsF.varspecs = { 'u':'gu*H(Z(mz,u,u0,nu,kz,gz),Z0u,nzu,lamdazu)*H(S,S0u,nsu,lamdaSu)*H(h,1,1,1)*H(A,1,1,1)-mz*Yu(u,u0,nu)-ku*u',
		   'mz':'gmz*H(Z(mz,u,u0,nu,kz,gz),Z0m,nzm,lamdaZm)*H(S,S0m,nsm,lamdaSm)*H(A,A0m,nAm,lamdaAm)-mz*Ym(u,u0,nu)-kmz*mz',
		   'ms':'gms*H(h,1,1,1)*H(S,S0ms,nsms,lamdaSms)*H(A,1,1,1)-ms*Ym3(u3,u30,nu3)-kms*ms',
...
def plotCC(x,y):
	...
	el= [0,0.1,0.3,0.5,0.7,1.,2.]
	ll,labs=[],[]
	minv,maxv=1000000,0
	for i in range(len(el)):
		DSargsF.pars['lamdaAm']=el[i]
...
if myrank==0:
	...
	plt.savefig("EM_AZ_hmz.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==1:
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AZ_hs.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==2:
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AZ_hu.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==3:
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AZ_hms.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==4:
	...
	plt.legend(ll,labs,fontsize=20,frameon=False,bbox_to_anchor=(1.04,1))
	plt.savefig("EM_AZ_hu3.png",bbox_inches='tight')
	#plt.show()
	plt.close()
