...
def fms(mz,Z,S,ms,u,u3,h,A,rm,rn):
	return gms*H(h,h0hs,lamdahs,nhs)*H(h,1,1,1)*H(S,S0ms,lamdaSms,nsms)*H(A,A0s,lamdaAs,nAs)-ms*Ym(u3,u30,nu3)-kms*ms
...
def fu(mz,Z,S,ms,u,u3,h,A,rm,rn):
	return gu*H(h,h0hu,lamdahu,nhu)*H(A,A0Au,lamdaAu,nAu)*H(Z,Z0u,lamdazu,nzu)*H(S,S0u,lamdaSu,nsu)*H(h,1,1,1)*H(A,1,1,1)-mz*Yu(u,u0,nu)-ku*u
...
def fRMT(mz,Z,S,ms,u,u3,h,A,rm,rn):
	return grm*H(u3,u0u3R,nu3mR,lamdau3mR)*H(A,A0aR,lambdaar,nar)*CompRmt(y,gn,h,h0hrm,nhrm,A,A0rm,narm)-krm*rm
...
def fH(mz,Z,S,ms,u,u3,h,A,rm,rn):
	R = rm+rn
	return gh*H(u,u0uh,nuh,lamdauh)*H(A,A0ah,lambdaah,nah)-kh*h*H(R,R0rh,lambdarh,nrh)*H(h,h0hh,lambdahh,nhh)
#############################################
#############################################
...
for i in range(nprocs):
	l_rank[i]=[]
count=0
for el in [0.2,0.4,0.6,0.75,0.8,0.85,1.]:
	l_rank[count%nprocs]+=[el]
	count+=1

for lamdauh in l_rank[myrank]:
	file1 = open("ics_Full_uH"+str(lamdauh)+".txt","w")
	file2 = open("res_Full_uH"+str(lamdauh)+".txt","w")
