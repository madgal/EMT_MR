...
def fmz(mz,Z,S,ms,u,u3,h,A,rm,rn):
	return gmz*H(Z,Z0m,lamdaZm,nzm)*H(S,S0m,lamdaSm,nsm)-mz*Ym(u,u0,nu)-kmz*mz
def fu(mz,Z,S,ms,u,u3,h,A,rm,rn):
	return gu*H(Z,Z0u,lamdazu,nzu)*H(S,S0u,lamdaSu,nsu)*H(h,1,1,1)*H(A,1,1,1)-mz*Yu(u,u0,nu)-ku*u
...
for i in range(nprocs):
	l_rank[i]=[]
count=0
for el in [0,0.2,0.3,0.35,0.4,0.6,1.]:
	l_rank[count%nprocs]+=[el]
	count+=1

for lamdaAs in l_rank[myrank]:
	file1 = open("ics_Full_AS"+str(lamdaAs)+".txt","w")
	file2 = open("res_Full_AS"+str(lamdaAs)+".txt","w")
