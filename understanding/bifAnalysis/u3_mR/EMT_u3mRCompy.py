...
DSargsF.pars = { 'ga':ga,'ka':ka,'A0aa':A0aa,'A0ah':A0ah,'A0aR':A0aR,'A0rn':A0rn,'A0rm':A0rm,'lambdaaa':lambdaaa,'lambdaah':lambdaah,'lambdaar':lambdaar,'y':y,
		...
		'y':8,'kh':0.25,'u0uh':10000.,'nuh':1,'lamdauh':0.8,'u0u3R':10000,'nu3mR':4,'lamdau3mR':2.,'u3':1000,'u':1000
		}
	
DSargsF.varspecs = { 'A':'ga*H(R(Rmt,Rnox),R0ra,nra,lambdara)*H(h,h0ha,nha,lambdaha)*H(A,A0aa,naa,lambdaaa)-ka*A',
		   'Rmt':'grm*H(A,A0aR,nar,lambdaar)*H(u3,u0u3R,nu3mR,lamdau3mR)*CompRmt(y,gn,h,h0hrm,nhrm,A,A0rm,narm)-krm*Rmt',
...
if myrank==0:
	...
	plt.legend(ll,labs)
	plt.savefig("EM_u3mR_3h.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==1:
	...
	plt.legend(ll,labs)
	plt.savefig("EM_u3mR_3A.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==2:
	...
	plt.ylabel("HIF1 ",fontsize=20)
	plt.legend(ll,labs)
	plt.savefig("EM_u3mR_AH.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==3:
	...
	plt.legend(ll,labs)
	plt.savefig("EM_u3mR_3rn.png",bbox_inches='tight')
	#plt.show()
	plt.close()
...
if myrank==4:
	...
	plt.legend(ll,labs)
	plt.savefig("EM_u3mR_3rm.png",bbox_inches='tight')
	#plt.show()
	plt.close()
