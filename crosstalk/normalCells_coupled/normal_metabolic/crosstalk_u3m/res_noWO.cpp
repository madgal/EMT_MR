...
std::string get3mParameters(int i,int j){
    std::string tmp_return="_u3m_0_";
    std::string s;
	lamda3m=i*0.1+j*0.01;
	s = std::to_string(i);
	tmp_return+=s.c_str();
	s = std::to_string(j);
	tmp_return+=s.c_str();
	     ...
	     for(int c=10;c<11;c++){//AZ set=10 for nothing
	       for(int d=1;d<2;d++){//HS set=1 for nothing
	         for(int e=10;e<11;e++){//Hu set=10 for nothing
	           for(int f=0;f<10;f++){//u3m set=10 for nothing
	           for(int f1=0;f1<10;f1++){//u3m set=10 f1or nothing
