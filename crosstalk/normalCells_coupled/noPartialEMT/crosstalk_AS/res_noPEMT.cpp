...
double lamdazu=0.1	;
double lamdaSu=0.85	;//Diff from 0.1
double lamdaIm =10.;
double lamdaSms = 0.1;
double lamdazu3 = 0.2;
double lamdaSu3 = 0.1;
...
std::string getASParameters(int i,int j){
    std::string tmp_return="_AS_0_";
    std::string s;
	lamdaAms=i*0.1+j*0.01;
	s = std::to_string(i);
	tmp_return+=s.c_str();
	s = std::to_string(j);
	tmp_return+=s.c_str();
...
int  main(void){
	...
	lamdaSu=0.85	;//Diff from 0.1

	for(int a=0;a<11;a++){//AS set=10 for nothing
	for(int a1=0;a1<11;a1++){//AS set=10 for nothing
	         ...
	         for(int e=10;e<11;e++){//Hu set=10 for nothing
	           for(int f=10;f<11;f++){//u3m set=10 for nothing
			    ...
			    if (i==0){
					...
					std::string str_cat=getASParameters(a,a1);
