...
double krm = 5. ;
double krn = 5.;
double kmh=.158;//Different value from original coupled model
double kh=2.2;//Different value from original coupled model
...
double lamdazu=0.1	;
double lamdaSu=0.85	;//Diff
double lamdaIm =10.;
double lamdaSms = 0.1;
double lamdazu3 = 0.2;
double lamdaSu3 = 0.1;
double lamdaZm=7.5	;
double lamdaSm=17.	;//Diff
...
bioLevels getICS(){
	...
	bioLevels ics = {rf_u,rf_mz,rf_Z,rf_ms,rf_u3,rf_S,rf_A,rf_Rmt,rf_Rnox,rf_h,rf_mh};
	return ics;
}
...
std::string getHuParameters(int i){
...
int  main(void){
	...
	lamdaSu=0.85	;//Diff
	lamdaSm=17.	;//Diff
	y=6.;//confirm it's normal cells
	kmh=.158;//Different value from original coupled model
	kh=2.2;//Different value from original coupled model
	...
	for(int e=0;e<3;e++){//Hu 
	  for(int f=0;f<5;f++){//u3m set=10 for nothing
	    for(int g=1;g<4;g++){//u3n set=10 for nothing
	      for(int h=3;h<8;h++){//input set=5 for nothing
	        for(int i=0;i<3;i++){//uh
               
                strcpy(finame,"EMT_MR10_HS_1_1_Au_1_1_AZ_0_95_AS_0_95_input_");
                strcpy(frname,"EMT_MR10_HS_1_1_Au_1_1_AZ_0_95_AS_0_95_input_");
		...
		switch(e){
			case 0:
	       			str_cat=getHuParameters(1);
	       			strcat(finame,str_cat.c_str());
	       			strcat(frname,str_cat.c_str());
				break;
			case 1:
	       			str_cat=getHuParameters(8);
	       			strcat(finame,str_cat.c_str());
	       			strcat(frname,str_cat.c_str());
				break;
			case 2:
	       			str_cat=getHuParameters(9);
	       			strcat(finame,str_cat.c_str());
	       			strcat(frname,str_cat.c_str());
				break;
		}
