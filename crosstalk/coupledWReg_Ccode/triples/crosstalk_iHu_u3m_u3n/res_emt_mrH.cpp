...
int  main(void){
	...
	// lamdahu =1, 1.045, 1.1,1.4,1.7,3,6
	lamdahu=1.;
	// lamda3m = 0.1,0.244,0.3,0.397,0.4
	lamda3m=1;
	// lamda3n = 0.,0.3,0.505,0.76
	lamda3n=1;

	for(int i=0; i<5;i++){
		for (int j=0; j<5;j++){
		    for (int k=0; k<4;k++){
			strcpy(finame,"EMT_MR_comp_Hu_");
			strcpy(frname,"EMT_MR_comp_Hu_");
			int tmpi,tmpj,tmpk,tmpl;
			std::string s;
			switch(i){
			    case 0:
				tmpj=1;
				tmpk=0;
				tmpl=0;
				lamdahu = tmpj*0.1+tmpk*0.01+tmpl*0.001;
				s = "0_100_u3m_";
				strcat(finame,s.c_str());
				strcat(frname,s.c_str());
				break;
			...
			switch(j){
				...
				tmpk=0;
				tmpl=0;
				lamda3m = tmpi*1.+tmpj*0.1+tmpk*0.01+tmpl*0.001;
				s = "0_100_u3n_";
				strcat(finame,s.c_str());
				strcat(frname,s.c_str());
				break;
			...
			switch(k){
				...
				tmpk=0;
				tmpl=0;
				lamda3n = tmpi*1.+tmpj*0.1+tmpk*0.01+tmpl*0.001;
				s = "0_000";
				strcat(finame,s.c_str());
				strcat(frname,s.c_str());
				break;
