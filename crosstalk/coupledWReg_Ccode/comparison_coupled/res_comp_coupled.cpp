...
std::string getCoupledParameters(int a){
    ...
    int i1b=0,i2b=0,j1b=0,j2b=0,j2c=0;
    switch(a){
	case 0://none
		i1=10.;//AS
		i2=10.;//AZ
	...
	std::cout<<lamdaAms<<','<<lamdaAm<<','<<lamdahu<<','<<lamda3m<<','<<lamda3n<<','<<lamdahms<<','<<lamdaAu<<','<<I<<'\n';
...
int  main(void){
	...
	for(int setI=0;setI<11;setI++){
		for(int a=4;a<5;a++){//a=4 for all coupled some of everything
			std::string str_cat;
			std::string s;
			/*
			strcpy(finame,"EMT_MR10");
			strcpy(frname,"EMT_MR10");
			str_cat=getCoupledParameters(a);
			strcat(finame,str_cat.c_str());
			strcat(frname,str_cat.c_str());
			s = "_";
			strcat(finame,s.c_str());
			strcat(frname,s.c_str());
			s = std::to_string(setI);
			strcat(finame,s.c_str());
			strcat(frname,s.c_str());
			srand(seeds[my_rank+setI*comm_sz]);
			...
			*/
			...
			strcat(frname,s.c_str());
			srand(seeds[my_rank+setI*comm_sz]);
			runSimulation(finame,frname,5000,my_rank,comm_sz);
