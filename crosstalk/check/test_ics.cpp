...
void runSimulation(int numICS,int my_rank, int comm_sz){
	
	MPI_File f_i;
	MPI_File f_r;
        char finame[100]="";
        strcat(finame,"ics.txt");
	...
	for(int i=0; i< timeL; i++){
		...
		double h = ics.h;
		double mh = ics.mh;
		snprintf(tempi_string,sizeof(tempi_string),"%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f\n",u,mz,Z,ms,u3,S,A,Rmt,Rnox,h,mh);
		std::cout<<tempi_string;
		MPI_File_write_shared(f_i, tempi_string, strlen(tempi_string), MPI_CHAR, MPI_STATUS_IGNORE);
	}
	//char tempi_string[100]="";
...
int  main(void){
	...
	for(int i=1; i<=1; i++){
		...
		runSimulation(10,my_rank,comm_sz);
	}//end for
