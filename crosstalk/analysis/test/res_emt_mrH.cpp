...
bioLevels getICS(){
	...
	double tmp_u= rand()%(u_range/100)*100.*1.1;
	double tmp_mz= rand()%(mz_range/100)*100.*1.2;
	double tmp_ms= rand()%(ms_range/100)*100.;
	double tmp_u3= rand()%(u3_range/100)*100.*1.1;
	double tmp_Z= rand()%(Z_range/100)*100.*1.2;
	double tmp_S= rand()%(S_range/100)*100.*0.9;
	double tmp_A= rand()%(A_range/100)*100.*1.1;
	double tmp_Rmt= rand()%(Rmt_range/100)*100.*1.2;
	double tmp_Rnox= rand()%(Rnox_range/100)*100.*1.1;
	double tmp_h= rand()%(h_range/100)*100.*1.2;
	double tmp_mh= rand()%(mh_range/100)*100.*0.9;

	bioLevels ics = {tmp_u,tmp_mz,tmp_Z,tmp_ms,tmp_u3,tmp_S,tmp_A,tmp_Rmt,tmp_Rnox,tmp_h,tmp_mh};
	return ics;
}
...
int  main(void){
	...
	for(int i=0; i<1;i++){
		for (int j=0; j<1;j++){
			...
			// for lamda3m = 0.ij
			//EMT_MR_comp_0_ij_1000_res.txt
			lamda3m=i*0.1+j*0.01+k*0.001;
			strcpy(finame,"EMT_MR_comp_u3m_0_");
			strcpy(frname,"EMT_MR_comp_u3m_0_");
