...
if nprocs!=6:
	print("Need 6 processes")
	exit()

lambdaList= list(lambdaVals.keys())
#print(lambdaList)
lamda = lambdaList[myrank]
print(lamda)
for lamI in range(len(lambdaVals[lamda])):
	...
	fileO = open("data/fp_"+lamda+"_"+str(lambdaVals[lamda][lamI])+".txt",'w')
	...
	fileO = open("data/nullU_"+lamda+"_"+str(lambdaVals[lamda][lamI])+".txt",'w')
	fileO.write('S,ms,mz,u,u3,A,h,Rnox,Rmt,mh\n')
	for i in range(len(solC_u['u'])):
		fileO.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" %( solC_u['S'][i], solC_u['ms'][i], solC_u['mz'][i], solC_u['u'][i], solC_u['u3'][i], solC_u['A'][i], solC_u['h'][i], solC_u['Rnox'][i], solC_u['Rmt'][i], solC_u['mh'][i]))
	fileO.close()


	solC_mz = nullclines('mz',DSargsF,fixed_points,new_domains,1000)
	fileO = open("data/nullmz_"+lamda+"_"+str(lambdaVals[lamda][lamI])+".txt",'w')
	fileO.write('S,ms,mz,u,u3,A,h,Rnox,Rmt,mh\n')
	for i in range(len(solC_mz['u'])):
		fileO.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" %( solC_mz['S'][i], solC_mz['ms'][i], solC_mz['mz'][i], solC_mz['u'][i], solC_mz['u3'][i], solC_mz['A'][i], solC_mz['h'][i], solC_mz['Rnox'][i], solC_mz['Rmt'][i], solC_mz['mh'][i]))
	fileO.close()


	solC_a = nullclines('A',DSargsF,fixed_points,new_domains,1000)
	fileO = open("data/nullA_"+lamda+"_"+str(lambdaVals[lamda][lamI])+".txt",'w')
	fileO.write('S,ms,mz,u,u3,A,h,Rnox,Rmt,mh\n')
	for i in range(len(solC_a['u'])):
		fileO.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" %( solC_a['S'][i], solC_a['ms'][i], solC_a['mz'][i], solC_a['u'][i], solC_a['u3'][i], solC_a['A'][i], solC_a['h'][i], solC_a['Rnox'][i], solC_a['Rmt'][i], solC_a['mh'][i]))
	fileO.close()


	solC_h = nullclines('h',DSargsF,fixed_points,new_domains,1000)
	fileO = open("data/nullH_"+lamda+"_"+str(lambdaVals[lamda][lamI])+".txt",'w')
	fileO.write('S,ms,mz,u,u3,A,h,Rnox,Rmt,mh\n')
	for i in range(len(solC_h['u'])):
		fileO.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" %( solC_h['S'][i], solC_h['ms'][i], solC_h['mz'][i], solC_h['u'][i], solC_h['u3'][i], solC_h['A'][i], solC_h['h'][i], solC_h['Rnox'][i], solC_h['Rmt'][i], solC_h['mh'][i]))
	fileO.close()
