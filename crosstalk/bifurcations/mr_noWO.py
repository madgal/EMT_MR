...
DSargsF.xdomain = {'A':[0,1000],'h':[0,1000],'Rmt':[0,150],'Rnox':[0,50],'mh':[0,1000]}
DSargsF.tdomain = [0,9000]
DSargsF.algparams = {'init_step':0.1}
new_domains = {'A':[0,1000],'h':[0,1000],'Rmt':[0,150],'Rnox':[0,50],'mh':[0,1000]}
...
### nullclines
solC_u = nullclines('A',DSargsF,fixed_points,new_domains,4000,step=1e+1)
try:
	fileO = open("data/nullA_noWO.txt",'w')
	fileO.write('A,h,Rmt,Rnox,mh\n')
...
### nullclines
solC_u = nullclines('h',DSargsF,fixed_points,new_domains,4000,step=1e+1)
try:
	fileO = open("data/nullh_noWO.txt",'w')
	fileO.write('A,h,Rmt,Rnox,mh\n')
	for i in range(len(solC_u['mh'])):
		fileO.write("%s,%s,%s,%s,%s\n" %( solC_u['A'][i], solC_u['h'][i], solC_u['Rmt'][i], solC_u['Rnox'][i], solC_u['mh'][i]))
	fileO.close()
