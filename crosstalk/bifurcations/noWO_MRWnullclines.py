...
DSargsF.pars ={'gz':100.,'gu':2100.,'grn':40.,'grm':150.,'gms':90.,'gu3':1350.,'gs':100.,'ga':30.,'gn':0.2,'g1':5.,'g2':0.2,'gmh':10.,'gh':1.5,'gmz':11.,'y':6.,
		'ks':0.125,'ka':0.2,'kz':0.1,'ku':0.05,'kmz':0.5,'kms':0.5,'ku3':0.05,'krm':5.,'krn':5.,'kmh':.158,'kh':2.2,
...
DSargsF.ics = {'h':100,'A':100,'Rnox':80,'Rmt':100,'mh':100}
DSargsF.xdomain = {'A':[0,500],'h':[0,500],'Rmt':[0,200],'Rnox':[0,100],'mh':[0,700]}
DSargsF.tdomain = [0,9000]
DSargsF.algparams = {'init_step':0.1}
new_domains = {'A':[0,500],'h':[0,500],'Rmt':[0,200],'Rnox':[0,100],'mh':[0,700]}
...
fig = plt.figure()
solC_h = nullclines('h',DSargsF,fixed_points,new_domains,2000,step=1e+1)
solC_a = nullclines('A',DSargsF,fixed_points,new_domains,2000,step=1e+1)
...
for i in range(len(fixed_points)):
	plt.plot(fixed_points[i]['A'],fixed_points[i]['h'],'ko',markersize=10)
fig.savefig("nullclines_figures/noWO_mr_null.png",bbox_inches='tight')
#plt.show()
plt.close()
