...
def reduce_fp(tmp_Ffp):
    ...
    for i in range(len(finalV)):
        finalArr+=[{'u':finalV[i][iu],'mz':finalV[i][imz],'u3':finalV[i][iu3],'S':finalV[i][iS],'ms':finalV[i][ims],'mg':finalV[i][img],'mo':finalV[i][imo],'G':finalV[i][iG],'O':finalV[i][iO],'T':finalV[i][it]}]
    return finalArr
...
DSargsF.varspecs = {'u':'gu*H(Z(mz,u,u0,nu,kz,gz),Z0u,nzu,lamdazu)*H(S,S0u,nsu,lamdaSu)*H(h,h0u,nhu,lamdahu)*H(A,A0u,nAu,lamdaAu)*H(O,o0u,nou,1.)-mz*Yu(u,u0,nu)-mh*Yuh(0,u0,nuh)-ku*u',
		   ...
		   'ms':'gms*H(S,S0ms,nsms,lamdaSms)*H(I,I0m,nIm,lamdaIm)*H(h,h0ms,nhms,lamdahms)*H(A,A0ms,nAms,lamdaAms)*H(T,t0m,ntm,1.)-ms*Ym(u3,u30,nu3)-kms*ms',
                   ...
                   'G':'gg*mg-kgg*G',
                   'O':'gov*mo-kov*O',
                   'T':'gt*H(O,o0mt,novmt,lomt)*H(u,u0mt,numt,lumt)-kt*T',
		}
...
DSargsF.pars ={'gz':100.,'gu':2100.,'gms':90.,'gu3':1350.,'gs':100.,'gmz':11.,
		...
		'lou':0.1,'loz':0.1,'lomo':0.1,'lzmo':0.5,'lzmg':0.5,'lgmz':0.1,
                ...
                'o0mt':25000.,'G0mo':25000.,'t0m':100.,'u0mt':10000.,
                'novmt':1,'nGmo':2,'ntm':1,'numt':1,
                'lomt':0.8,'lGmo':0.7,'ltm':12.5,'lumt':0.8 } 
##############
##############
#####
DSargsF.ics={'u':1000,'mz':1000,'S':200000,'ms':2000,'u3':4000,'G':90000.,'O':30000.,'T':200.,'mg':50.,'mo':20.}#'Z':1000}
DSargsF.xdomain = {'u':[0,100000],'mz':[0,2000],'S':[0,600000],'ms':[0,20000],'u3':[0,50000],'G':[0,200000],'O':[0,100000],'T':[0,1000],'mg':[0,1000],'mo':[0,1000]}
DSargsF.tdomain = [0,8000]
DSargsF.algparams = {'init_step':0.4}
new_domains = {'u':[0,100000],'mz':[0,2000],'S':[0,600000],'ms':[0,20000],'u3':[0,50000],'G':[0,200000],'O':[0,100000],'T':[0,1000],'mg':[0,1000],'mo':[0,1000]}
...
#plt.show()
fig.savefig("nullclines_figures/PSF_ia_emt_nullcline.png",bbox_inches='tight')
plt.close()
