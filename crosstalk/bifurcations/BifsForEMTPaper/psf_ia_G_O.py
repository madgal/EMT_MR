...
DSargsF = PyDSTool.args(name='psf_iaFull',checklevel=2)
...
DSargsF.varspecs = {'u':'gu*H(Z(mz,u,u0,nu,kz,gz),Z0u,nzu,lamdazu)*H(S,S0u,nsu,lamdaSu)*H(O,o0u,nou,1.)-mz*Yu(u,u0,nu)-mh*Yuh(0,u0,nuh)-ku*u',
		   'mz':'gmz*H(Z(mz,u,u0,nu,kz,gz),Z0m,nzm,lamdaZm)*H(S,S0m,nsm,lamdaSm)*H(O,o0z,noz,1.)*H(G,g0mz,ngmz,1.)-mz*Ym(u,u0,nu)-kmz*mz',
...
##############
#####
DSargsF.ics={'u':1000,'mz':5000,'S':200000,'ms':200,'u3':4000,'G':70000.,'O':20000.,'mg':30.,'mo':10.}#'Z':1000}
DSargsF.xdomain = {'u':[0,20000],'mz':[0,1000],'S':[0,250000],'ms':[0,500],'u3':[0,20000],'G':[0,90000],'O':[0,30000],'mg':[0,50],'mo':[0,20]}
DSargsF.tdomain = [0,8000]
DSargsF.algparams = {'init_step':0.4}
new_domains = {'u':[0,20000],'mz':[0,1000],'S':[0,250000],'ms':[0,500],'u3':[0,20000],'G':[0,90000],'O':[0,30000],'mg':[0,50],'mo':[0,20]}
...
fileO = open("data/PSF_ia_fp.txt",'w')
...
try:
	fileO = open("data/nullU_psf_ia.txt",'w')
...
try:
	fileO = open("data/nullMZ_psf_ia.txt",'w')
	fileO.write('S,ms,mz,u,u3\n')
	for i in range(len(solC_u['S'])):
		fileO.write("%s,%s,%s,%s,%s,%s,%s,%s,%s\n" %( solC_u['S'][i], solC_u['ms'][i], solC_u['mz'][i], solC_u['u'][i], solC_u['u3'][i], solC_u[i]['G'], solC_u[i]['O'], solC_u[i]['mg'], solC_u[i]['mo']))
	fileO.close()
